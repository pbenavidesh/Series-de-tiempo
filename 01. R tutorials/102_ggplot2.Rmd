...
Para ver la versión renderizada del código, hay que hacer clic en *Preview*.
...
ggplot(economics %>% filter(year(date)>=2006)) + 
  ...
  geom_point(aes(x = date, y = unemploy), size = 1, color ="green")
```

Se pueden crear sus propias `tibbles` directamente en **R** con la función `tibble()`:

```{r}
tibble(fecha = c("2020-05-27","2020-05-28","2020-05-29"),
       dolar = c(24,24.3,25),
...
Podemos ordenar una tabla con respecto a una de sus variables con `arrange()`. Por defáult, el orden será ascendente. Si se desea en orden descendente, agreamos `desc()`. Probemos mostrando a los países que tienen la mayor población en el año 2007.
...
p1 <- gapminder %>% 
  ...
  labs(x = "Año", y = "Esperanza de vida (años)", title = "Evolución de la esperanza de vida en el tiempo")
