...
ggplot(data = mpg) + 
  ...
  facet_grid(drv ~ cyl) +
  xlab("Tamaño del motor (en litros)") +
  ylab("Rendimiento en carretera (millas por galón)")
```

## {-}

## Gráficas de líneas {.tabset .tabset-pills}
...
# Para saber cuántos países distintos tenemos en la tabla

gapminder %>% 
  nrow()

gapminder %>% # %>% = "toma la tabla `gapminder`, luego"
  distinct(country) %>% # "quédate solo con los diferentes países", luego
  nrow() # dime cuántos son

# nrow(distinct(gapminder, country)) # manera tradicional
...
# Para quedarnos con los países de África en el año 1967
# Opción 1
gapminder %>% 
  filter(year == 1967) %>% 
...
# Para quedarnos con los países asiáticos o europeos
# Opción incorrecta
gapminder %>% 
  filter(continent == "Asia") %>% 
  filter(continent == "Europe") # primero está filtrando por Asia y luego por Europa, por lo tanto, hay cero filas que correspondan
...
# Quedarnos con los países Mexico, Alemania, EEEUU, Canada, Brasil y Chile

gapminder %>% 
  filter(country == "Mexico" | country == "Germany" |
           country == "United States" | country == "Canada" | country == "Brazil" | country == "Chile")

# Opción 2
lista_paises <- c("Mexico", "Germany", "United States", "Canada", "Brazil", "Chile", "Turkey", "France")

gapminder %>% 
  filter(country %in% lista_paises)

gapminder %>% 
  filter(country %in% c("Mexico", "Germany", "United States", "Canada", "Brazil", "Chile"))
...
# Para quedarnos solo con las filas 11-15
gapminder %>% 
  slice(11:15)


# Para quedarnos con las primeras 20 filas
gapminder %>% 
  slice_head(n = 20)

# Para saber el top 10 de países con la mayor esperanza de vida en 2007
gapminder %>% 
  filter(year == 2007) %>% 
  slice_max(order_by = lifeExp, n = 10)
...
gapminder %>% 
  ...
  ggplot(aes(x = year, y = Poblacion_total, color = continent)) +
  geom_line()
```
...
tabla %>%
  ...
  geom_line() + scale_y_continuous(name = "Población total", labels = comma)
```
