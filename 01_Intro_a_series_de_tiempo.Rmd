---
title: "Introduccion a series de tiempo y pronósticos"
output:
  html_notebook:
    toc: yes
    toc_float: yes
...
Hay muchos fenómenos o variables que se pueden pronosticar; p. ej. los insumos que va a requerir una planta el siguiente mes o semana, las ventas de cierta sucursal en el mediano o largo plazo, la demanda de energia para una region del país en las siguientes 24 horas, etc.
...
```{r tabla,echo=FALSE}
kable(data.frame(Año = c(2012:2020), Observación = c(119, 41, 92, 48, 115, 33, 27, 87, 103)))
...
Observando el resultado, vemos que **R** en automatico detecto el inicio, `r start(y)[1]`, y fin de la serie, `r end(y)[1]`. Adicionalmente, nos imprimio el argumento de `frequency`. La **frecuencia** de una serie de tiempo es el numero de observaciones que existen antes de que se repita el patron estacional.
...
```{r paqueterias, results='hide', warning=FALSE, message=FALSE}
library("tidyverse")
library("fpp2")

```
```{r passengers plot, results='markup'}
