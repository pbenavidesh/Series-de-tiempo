...
inputPanel(
  ...
  numericInput("ma_q",label = "q",value = 0, min = 0, max = 5, step = 1)
), fluidRow(
...
modelo <- eventReactive(input$go, {
  ho2 %>% 
    model(ARIMA(log(Cost) ~ 0 + pdq(input$ar_p, input$i_d, input$ma_q) + PDQ(input$ar_P, input$i_D, input$ma_Q)))
})

renderText({
  paste("Estimación del modelo ARIMA(",input$ar_p, ",", input$i_d, ",", input$ma_q, ")","(",input$ar_P, ",", input$i_D, ",", input$ma_Q, ")~12~" sep = "")
})
...
renderPlot({
  modelo() %>% gg_tsresiduals(lag_max = 36) + ggtitle("Diagnóstico de residuos"
  )
})
renderText({
  "Prueba de Ljung-Box"
...
renderPrint({
  modelo() %>% augment() %>% features(.resid, ljung_box, lag = 36, dof = nrow(tidy(modelo())))
})
