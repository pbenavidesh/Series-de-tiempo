...
packages("tidyverse", "tidyquant", "lubridate", "patchwork", "fpp2","fpp3","scales", "timetk", "plotly")
...
beer_ma <- beer %>%
  mutate(
    `4-MA` = slide_dbl(Beer, mean, .size = 4, .align = "center-right"),
...
> Graficamos todos los países y visualizamos con `plotly` para ver cuáles son los que tienen un mayor PIB per Cápita.
...
> Se puede cambiar la posición de la leyenda:

```{r}
ej1a <- ge %>%
  filter(Country %in% c("Mexico", "Monaco", "India")) %>% 
  ggplot(aes(x = Year, y = PIB_Per_Capita, color = Country)) +
  geom_line() + scale_y_log10()

ej1a + theme(legend.position = "top") +
ej1a + theme(legend.position = "bottom") +
ej1a + theme(legend.position = "left") +
ej1a + theme(legend.position = "right") #por default hace esto
```

> También pudimos haber agregado etiquetas para, p. ej., el top 5 de los países con mayor PIB per Cápita.

```{r}
max_gdp <- ge %>% 
  filter(Year == 2016) %>% 
  slice_max(PIB_Per_Capita, n = 1)
...
ej2 <- global_economy %>% 
  ...
  mutate(
    `PIB per Cápita` = GDP / Population,
    `PIB per Cápita deflactado` = `PIB per Cápita` / CPI
  ) %>% 
  autoplot(`PIB per Cápita`, linetype = 6) +
  scale_y_log10()
ggplotly(ej2)
```

> Se puede ajustar por población para obtener el PIB per Cápita y ajustar la inflación para ver el crecimiento de la economía en términos reales.
...
vic_elec %>%
  ...
  autoplot()
```
> Al realizar la descomposición de la serie vemos que presenta varios tipos de estacionalidad:
 * Anual (cada año se repite un patrón, debido principalmente a la temperatura).
 * Semanal (entre semana se utiliza más energía que los fines de semana, debido a la actividad económica).
 * Diaria (depende de la hora del día cuánta energía se requiere, en la madrugada se demanda menos energía, en la noche más).
 * Por hora (hay un patrón sistemático en la demanda dependiendo si es la hora en punto o media hora {habría que revisar más a profundidad esta}).
...
> Utilizando la característica de guerrero, obtenemos un lambda de `r lambda`, pero la gráfica no parece que logre estabilizar por completo la varianza.         
...
class(plastico)
```
> La tabla es un objeto `ts`, por lo que necesitamos convertirlo a un objeto `tsibble`.
...
> La serie parece tener una tendencia positiva y una estacionalidad anual.
...
plastico_dcmp <- plastico %>% 
  model(Descomposicion = classical_decomposition(value, type = "multiplicative")) %>% 
  components() 
...
> Sí coinciden. Aunque yo no había detectado la baja en la tendencia al final de la serie.
...
plastico %>% 
  ...
  autolayer(plastico_dcmp, season_adjust, color = "darkgreen")
```
...
> Voy a cambiar la observación 20

```{r}
obs <- 20
plastico2 <- plastico
plastico2$value[obs]
plastico2$value[obs] <- plastico2$value[obs] + 500
plastico2$value[obs]
plastico2
...
```{r}
plastic2_dcmp <- plastico2 %>% 
  model(dcmp = classical_decomposition(value, type = "multiplicative")) %>% 
  components()
...
plastico2 %>% 
  autoplot(value, color = "gray") +
  autolayer(plastic2_dcmp, season_adjust, color = "firebrick")
```
...
plastico2 <- plastico
plastico2$value[obs]
plastico2$value[obs] <- plastico2$value[obs] + 1000
plastico2$value[obs]
plastico2
